...
public class SlownessEffect : StatusEffect
{
    ...
    public override void OnStackCountChanged(GameObject target, int currentStacks)
    {
        // 1. Safely grab the specific StatusEffectManager attached to this target character
        if (target.TryGetComponent<StatusEffectManager>(out StatusEffectManager manager))
        {
    ...
    {
        // 1. Grab the specific StatusEffectManager attached to this target character
        if (target.TryGetComponent<StatusEffectManager>(out StatusEffectManager manager))
        {
